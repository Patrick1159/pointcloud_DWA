 ...
 * @return 所有候选轨迹及其对应的控制量和评分
 */
vector<tuple<vector<double>, vector<VectorXd>, double>> DWA::generateAllTrajectories(VectorXd state, Vector2d goal, vector<Vector2d> obstacle) {
    vector<tuple<vector<double>, vector<VectorXd>, double>> all_trajectories;
    
    // 计算动态窗口
    vector<double> dynamic_window_vel = calDynamicWindowVel(state(3), state(4), state, obstacle);
    ...
    // 设置为1.0表示不进行归一化，如果需要归一化可以使用上面计算的总和
    sum_heading = 1.0;
    sum_dist = 1.0;
    sum_vel = 1.0;
    ...
    while (v < dynamic_window_vel[1]) {
        ...
        while (w < dynamic_window_vel[3]) {
            ...
            double score = heading_eval + dist_eval + vel_eval;
            
            // 保存控制量、轨迹和评分
            all_trajectories.push_back(make_tuple(control, trajectory, score));
